...
def preprocess_librispeech(datasets_root: Path, out_dir: Path, n_processes: int, 
    ...
    input_dirs = [dataset_root.joinpath("2")]#,dataset_root.joinpath("train-clean-360")]
    ...
    func = partial(preprocess_speaker2, out_dir=out_dir, skip_existing=skip_existing, 
                   hparams=hparams)
...
def preprocess_sst(datasets_root: Path, out_dir: Path, n_processes: int, 
    ...
    speaker_dirs = input_dirs
    #print(speaker_dirs)
    func = partial(preprocess_speaker_sst, out_dir=out_dir, skip_existing=skip_existing, 
                   hparams=hparams)
    job = Pool(n_processes).imap(func, speaker_dirs)
    #print(job)	
    for speaker_metadata in tqdm(job, "LibriSpeech", len(speaker_dirs), unit="speakers"):
