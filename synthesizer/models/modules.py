    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if self.is_training:
            ...
            c = (1 - self._zoneout_cell) * tf.nn.dropout(new_c - prev_c,
                                                         rate=(1 - (1 - self._zoneout_cell))) + prev_c
            h = (1 - self._zoneout_outputs) * tf.nn.dropout(new_h - prev_h,
                                                            rate=(1 - (1 - self._zoneout_outputs))) + prev_h
...
def MaskedMSE(targets, outputs, targets_lengths, hparams, mask=None):
    ...
    with tf.control_dependencies([tf.compat.v1.assert_equal(tf.shape(targets), tf.shape(mask_))]):
...
def MaskedSigmoidCrossEntropy(targets, outputs, targets_lengths, hparams, mask=None):
    ...
    with tf.control_dependencies([tf.compat.v1.assert_equal(tf.shape(targets), tf.shape(mask))]):
    ...
    with tf.control_dependencies([tf.compat.v1.assert_equal(tf.shape(mask), tf.shape(losses))]):
