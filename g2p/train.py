...
class Net:
    ...
    def encode(self, xs):
        ...
        with tf.compat.v1.variable_scope("encode", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            cell = tf.compat.v1.nn.rnn_cell.GRUCell(self.hp.hidden_units)
            _, last_hidden = tf.compat.v1.nn.dynamic_rnn(cell, x, seqlens, dtype=tf.float32)
    ...
    def decode(self, ys, h0=None):
        ...
        with tf.compat.v1.variable_scope("decode", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            cell = tf.compat.v1.nn.rnn_cell.GRUCell(self.hp.hidden_units)
            outputs, last_hidden = tf.compat.v1.nn.dynamic_rnn(cell, inputs, initial_state=h0, dtype=tf.float32)

            # projection
            logits = tf.compat.v1.layers.dense(outputs, len(self.p2idx))
            y_hat = tf.cast(tf.argmax(logits, axis=-1), dtype=tf.int32)
    ...
    def train(self, xs, ys):
        ...
        nonpadding = tf.cast(tf.not_equal(y, self.p2idx["<pad>"]), dtype=tf.float32) # 0: <pad>
        loss = tf.reduce_sum(ce*nonpadding) / (tf.reduce_sum(nonpadding)+1e-7)

        global_step = tf.compat.v1.train.get_or_create_global_step()
        train_op = tf.compat.v1.train.AdamOptimizer(hp.lr).minimize(loss, global_step=global_step)
    ...
    def eval(self, xs, ys):
        ...
        n = tf.random.uniform((), 0, tf.shape(y_hats)[0]-1, tf.int32)
        word = words[n]
